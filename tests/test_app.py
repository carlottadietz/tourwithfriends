...
class TourWithFriendsTests(unittest.TestCase):
    ...
    def test_ceremony_dashboard_shows_podium_overtakes_and_stage_wins(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("Siegerehrungs-Dashboard".encode("utf-8"), response.data)
        self.assertIn(b"Top 3 pro Kategorie", response.data)
        self.assertIn("nacheinander aufgerufen".encode("utf-8"), response.data)
        self.assertIn("Kategorie wählen".encode("utf-8"), response.data)
        self.assertIn(b"Femme", response.data)
        self.assertIn(b"Homme", response.data)
        self.assertIn(b"Bea", response.data)
        self.assertIn(b"Anna", response.data)
        self.assertIn(b"Chris", response.data)
        self.assertIn("Verlauf pro Fahrer".encode("utf-8"), response.data)
        self.assertIn("Übersichtsleiste".encode("utf-8"), response.data)
        self.assertIn("X-Achse".encode("utf-8"), response.data)
        self.assertIn("Y-Achse".encode("utf-8"), response.data)
